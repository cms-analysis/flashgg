...
process.source = cms.Source ("PoolSource",fileNames = cms.untracked.vstring("file:/afs/cern.ch/user/s/sani/work/flashgg/src/flashgg/MicroAODProducers/test/myMicroAODOutputFile.root",
))

import flashgg.TagAlgos.dumperConfigTools as cfgTools
process.load("flashgg.TagProducers.diphotoMVAWithZeeDumper_cff")
process.load("flashgg.TagProducers.photonDumper_cfi")
process.load("flashgg.TagProducers.diphotonDumper_cfi") 
...
process.DiPhotonWithZeeMVADumper.nameTemplate ="$PROCESS_$SQRTS_$LABEL_$SUBCAT"
...
cfgTools.addCategories(process.photonDumper,
                       ...
                       histograms=["r9>>r9(110,0,1.1)",
                                   ...
                                   "covieip>>covieip(100, -0.00015, 0.00015)",
...
# DIPHOTON MVA INPUTS + ID MVA Chosen vtx
## define categories and associated objects to dump
...
cfgTools.addCategories(process.diphotonDumper,
                       ...
                       variables=["CMS_hgg_mass[120,60,120]:=mass", 
                       ...
                       histograms=["CMS_hgg_mass>>mass(120,60,120)",
...
process.p = cms.Path(process.DiPhotonWithZeeMVADumper
