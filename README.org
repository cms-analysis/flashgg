...
* HHWWgg
This section of the readme describes how to run modules specific to the HH->WWgg analysis. The current plugin designed to work with workspaceStd
...
from workspacestd.py. To run locally, after a scram b -j command to build everything, you need to run the command:
...
constructed from the highest sum pt squared vertex, found the be slightly more efficient in the case of the 250 GeV HHWWgg semileptonic signal compared to the Hgg vertex.
...
condor can be done with the script HHWWgg_Run_Jobs.sh. This allows you to run with the CandidateDumper or Tagger on signal or background. 
