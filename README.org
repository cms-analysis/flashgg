...
In order to submit jobs with 2017 Data, you would run the command:
    ...
    #+BEGIN_EXAMPLE
    . HHWWgg_Run_Jobs.sh --labelName HHWWgg_2017_Data_Trees --nEvents all --json Taggers/test/HHWWgg_2017_Data_All/HHWWgg_Data_All_2017.json --condorQueue longlunch -g -c -v -t
    #+END_EXAMPLE
...
simply submit with the command:
    ...
    #+BEGIN_EXAMPLE
    . HHWWgg_Run_Jobs.sh --labelName HHWWgg_2017_FggBackground_Trees --nEvents all --json Taggers/test/DataMC/Flashgg_bkg.json --condorQueue workday -g -c -v -t
    #+END_EXAMPLE
