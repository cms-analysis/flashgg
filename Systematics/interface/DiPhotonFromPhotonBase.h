...
namespace flashgg {
    ...
    public:
        DiPhotonFromPhotonBase( const edm::ParameterSet &conf, edm::ConsumesCollector && iC );
    ...
    template<class param_var>
    DiPhotonFromPhotonBase<param_var>::DiPhotonFromPhotonBase( const edm::ParameterSet &conf, edm::ConsumesCollector && iC ) :
        ...
        debug_( conf.getUntrackedParameter<bool>( "Debug", false ) )
    {
        ...
        photon_corr_.reset( FlashggSystematicMethodsFactory<flashgg::Photon, param_var>::get()->create( photonMethodName, conf, std::forward<edm::ConsumesCollector>(iC) ) );
        if(conf.exists("BinList2"))  //if defined, BinList2 gives bins for sublead, lead uses BinList
            {
                ...
                photon_corr2_.reset( FlashggSystematicMethodsFactory<flashgg::Photon, param_var>::get()->create( photonMethodName, conf2, std::forward<edm::ConsumesCollector>(iC) ) );
            
            }
        else //if BinList2 is not defined, use BinList for both lead and sublead photons
            photon_corr2_.reset( FlashggSystematicMethodsFactory<flashgg::Photon, param_var>::get()->create( photonMethodName, conf, std::forward<edm::ConsumesCollector>(iC) ) );
