...
namespace flashgg {
    ...
    class JetPUJIDShift: public ObjectSystMethodBinnedByFunctor<flashgg::Jet, int>
    {

    public:
        typedef StringCutObjectSelector<Jet, true> selector_type;

        JetPUJIDShift( const edm::ParameterSet &conf, edm::ConsumesCollector && iC, const GlobalVariablesComputer * gv );
    ...
    JetPUJIDShift::JetPUJIDShift( const edm::ParameterSet &conf, edm::ConsumesCollector && iC, const GlobalVariablesComputer * gv ) :
        ObjectSystMethodBinnedByFunctor( conf, std::forward<edm::ConsumesCollector>(iC), gv  ),
        overall_range_( conf.getParameter<std::string>( "OverallRange" ) )
    {
    }

    std::string JetPUJIDShift::shiftLabel( int syst_value ) const
    {
        std::string result;
        if( syst_value == 0 ) {
    ...
    void JetPUJIDShift::applyCorrection( flashgg::Jet &y, int syst_shift )
    {
        if( overall_range_( y ) ) {
            ...
            if( val_err.first.size() == 1 && val_err.second.size() == 1 ) { // otherwise no-op because we don't have an entry
                ...
                float theRms = y.puJetIdMVA();
                y.setSimpleMVA( theRms + shift );
                if ( this->debug_ ) {
                    std::cout << "JetPUJIDShift::applyCorrection syst_shift=" << syst_shift
                              << " old rms="        << theRms
                              << " new puJetIdMVA=" << y.puJetIdMVA()
                              << std::endl;
                }
            }
        }
...
DEFINE_EDM_PLUGIN( FlashggSystematicJetMethodsFactory,
                   flashgg::JetPUJIDShift,
                   "FlashggJetPUJIDShift" );
// Local Variables:
// mode:c++
