...
namespace flashgg{
  ...
  class MetJecSmear: public BaseSystMethod<flashgg::Met, int>
  {


  public:
    typedef StringCutObjectSelector<flashgg::Met, true> selector_type;
    MetJecSmear( const edm::ParameterSet &conf, edm::ConsumesCollector && iC, const GlobalVariablesComputer * gv );
  ...
  void MetJecSmear::eventInitialize( const edm::Event &iEvent, const edm::EventSetup & iSetup ) 
  {
    
  }
  MetJecSmear::MetJecSmear(const edm::ParameterSet &conf, edm::ConsumesCollector && iC, const GlobalVariablesComputer * gv) :
  ...
  std::string MetJecSmear::shiftLabel( int syst_value ) const
  {
    std::string result;
    if( syst_value == 0 ) {
  ...
  void MetJecSmear::applyCorrection( flashgg::Met &mets, int syst_shift )
  {
      ...
      if( overall_range_( mets ) ) 
          {
              ...
              if(syst_shift==0)
              ...
              else if(syst_shift==1)
                  {
                      ...
                      mets.setCorPt(mets.shiftedPt((pat::MET::METUncertainty) 2));
                      mets.setCorPhi(mets.shiftedPhi((pat::MET::METUncertainty) 2));
                  }
              else if(syst_shift==-1)
                  {
                      mets.setCorPt(mets.shiftedPt((pat::MET::METUncertainty) 3));
                      mets.setCorPhi(mets.shiftedPhi((pat::MET::METUncertainty) 3));
                  }
              else
                  {
                      std::cout << std::endl;
