...
namespace flashgg{
  ...
  class MetJerSmear: public BaseSystMethod<flashgg::Met, int>
  {


  public:
    typedef StringCutObjectSelector<flashgg::Met, true> selector_type;
    MetJerSmear( const edm::ParameterSet &conf, edm::ConsumesCollector && iC, const GlobalVariablesComputer * gv );
  ...
  void MetJerSmear::eventInitialize( const edm::Event &iEvent, const edm::EventSetup & iSetup ) 
  {
    
  }
  MetJerSmear::MetJerSmear(const edm::ParameterSet &conf, edm::ConsumesCollector && iC, const GlobalVariablesComputer * gv) :
  ...
  std::string MetJerSmear::shiftLabel( int syst_value ) const
  {
    std::string result;
    if( syst_value == 0 ) {
  ...
  void MetJerSmear::applyCorrection( flashgg::Met &mets, int syst_shift )
  {
      ...
      if( overall_range_( mets ) ) 
          {
              ...
              if(syst_shift==0)
              ...
              else if(syst_shift==1)
                  {
                      ...
                      mets.setCorPt(mets.shiftedPt((pat::MET::METUncertainty) 0));
                      mets.setCorPhi(mets.shiftedPhi((pat::MET::METUncertainty) 0));
                  }
              else if(syst_shift==-1)
                  {
                      mets.setCorPt(mets.shiftedPt((pat::MET::METUncertainty) 1));
                      mets.setCorPhi(mets.shiftedPhi((pat::MET::METUncertainty) 1));
                  }
              else
                  {
                      std::cout << std::endl;
