...
namespace flashgg{
  ...
  class MetUncSmear: public BaseSystMethod<flashgg::Met, int>
  {


  public:
    typedef StringCutObjectSelector<flashgg::Met, true> selector_type;
    MetUncSmear( const edm::ParameterSet &conf, edm::ConsumesCollector && iC, const GlobalVariablesComputer * gv );
  ...
  void MetUncSmear::eventInitialize( const edm::Event &iEvent, const edm::EventSetup & iSetup ) 
  {
    
  }
  MetUncSmear::MetUncSmear(const edm::ParameterSet &conf, edm::ConsumesCollector && iC, const GlobalVariablesComputer * gv) :
  ...
  std::string MetUncSmear::shiftLabel( int syst_value ) const
  {
    std::string result;
    if( syst_value == 0 ) {
  ...
  void MetUncSmear::applyCorrection( flashgg::Met &mets, int syst_shift )
  {
      ...
      if( overall_range_( mets ) ) 
          {
              ...
              if(syst_shift==0)
              ...
              else if(syst_shift==1)
                  {
                      ...
                      mets.setCorPt(mets.shiftedPt((pat::MET::METUncertainty) 10));
                      mets.setCorPhi(mets.shiftedPhi((pat::MET::METUncertainty) 10));
                  }
              else if(syst_shift==-1)
                  {
                      mets.setCorPt(mets.shiftedPt((pat::MET::METUncertainty) 11));
                      mets.setCorPhi(mets.shiftedPhi((pat::MET::METUncertainty) 11));
                  }
              else
                  {
                      std::cout << std::endl;
