...
namespace flashgg {
    ...
    class PhotonScaleStringNew: public ObjectSystMethodBinnedByFunctor<flashgg::Photon, int>
    {

    public:
        typedef StringCutObjectSelector<Photon, true> selector_type;

        PhotonScaleStringNew( const edm::ParameterSet &conf );
    ...
    PhotonScaleStringNew::PhotonScaleStringNew( const edm::ParameterSet &conf ) :
    ...
    std::string PhotonScaleStringNew::shiftLabel( int syst_value )
    {
        std::string result;
        if( syst_value == 0 ) {
    ...
    void PhotonScaleStringNew::applyCorrection( flashgg::Photon &y, int syst_shift )
    {
        if( overall_range_( y ) ) {
...
DEFINE_EDM_PLUGIN( FlashggSystematicPhotonMethodsFactory,
                   flashgg::PhotonScaleStringNew,
                   "FlashggPhotonScaleStringNew" );
// Local Variables:
// mode:c++
