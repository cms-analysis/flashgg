...
def customizeJetSystematicsForData(process):
    ...
    process.jec.toGet[0].tag = cms.string(process.jec.toGet[0].tag.value().replace("DATA","MC"))
    process.jec.connect = cms.string(process.jec.connect.value().replace("Data","MC"))
    from flashgg.Taggers.flashggTags_cff import UnpackedJetCollectionVInputTag
