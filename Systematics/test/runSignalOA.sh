...
fggRunJobs.py --load sig_jobs_OA_2016.json -d ${outDir}_signal_OA_16 -x cmsRun fiducialStd.py metaDataSrc=flashgg useAAA=True dumpWorkspace=False dumpTrees=True doSystematics=False doJets=True doBJetsAndMET=False puTarget=${cat lumi/PU2016.txt}  maxEvents=-1 --stage-to=/eos/user/t/threiten/Analysis/Differentials/2016ReReco/${outDir}_signal_OA_16/ -q tomorrow -n 50 -H --no-copy-proxy &

mkdir -p /eos/user/t/threiten/Analysis/Differentials/2017ReReco/${outDir}_signal_OA_17/
fggRunJobs.py --load sig_jobs_OA_2017.json -d ${outDir}_signal_OA_17 -x cmsRun fiducialStd.py metaDataSrc=flashgg useAAA=True dumpWorkspace=False dumpTrees=True doSystematics=False doJets=True doBJetsAndMET=False puTarget=${cat lumi/PU2017.txt} maxEvents=-1 --stage-to=/eos/user/t/threiten/Analysis/Differentials/2017ReReco/${outDir}_signal_OA_17/ -q tomorrow -n 50 -H --no-copy-proxy &
