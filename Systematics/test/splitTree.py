...
def main(options):
    ...
    variables = parse_variables(config['variables'])
    print(variables)
    cut = config['cut']
    ...
    for cat in config['categories']:
        ...
        proc = config['proc']  #FIXME

        if 'formulas' in config.keys():
            columns = None
        else:
            columns = splitDic.keys() + [var[0] for var in variables] + ['leadmva', 'subleadmva']
            
        df = root_pandas.read_root(options.infile, '{}/{}_{}'.format(config['treepath'], proc, cat), columns=columns)
    ...
    write_cat_procs_file('{}/proc_cat_names_{}.txt'.format(outfolder,splitDic.keys()[0]), procs, cats)
...
if __name__ == "__main__":
    ...
    requiredAgrs.add_argument('--infile', '-i', action='store', type=str, required=True)
    requiredAgrs.add_argument('--outfolder', '-o', action='store', type=str, required=True)
    requiredAgrs.add_argument('--config', '-c', action='store', type=str, required=True)
