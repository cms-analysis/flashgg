...
do
echo '###############'
pre_name=`grep 'outputFile=' ./$1runJobs${num_job}.sh |awk '{print$17}'`
# pre_name=`grep 'outputFile=' ./$1runJobs${num_job}.sh`
echo prename: $pre_name
...
echo job:${num_job}
echo files:$num_file
pre_num=`grep -r nJobs ./$1runJobs${num_job}.sh |awk '{print$18}'`
# pre_num=`grep -r nJobs ./$1runJobs${num_job}.sh`
